...
def radians(degreeValue):
    # Convert the degreeValue into radians
    radianValue =  degreeValue * pi / 180.
    return radianValue
...
def get_solarTime(when, longitude):
    ...
    if alreadySolartime:
        ...
        meridian = abs(15 * (time.timezone) / 3600)
        if longitude > 0:
...
def get_Kt(temperature, humidity, cloudCover):
    ...
    Kt = 0.428806746933 + (0.00957248670459 * temperature) +  (-0.00268188151295 * humidity) +  (0.00168692434981 * cloudCover)

    return Kt  # clearness index

def get_G(when, latitude, longitude, temperature, humidity, cloudCover):
    Kt = get_Kt(temperature, humidity, cloudCover)
    Go = get_outerBeamH(when, latitude, longitude)
    G = Kt * Go
    return G

def get_Gd(when, latitude, longitude, temperature, humidity, cloudCover):
    try:
        G = get_G(when, latitude, longitude, temperature, humidity, cloudCover)
        Kt = get_Kt(temperature, humidity, cloudCover)
...
def get_Gb(when, latitude, longitude, temperature, humidity, cloudCover):
    G = get_G(when, latitude, longitude, temperature, humidity, cloudCover)
    Gd = get_Gd(when, latitude, longitude, temperature, humidity, cloudCover)
    Gb = G - Gd
    return Gb

def get_GtIsotropic(when, latitude, longitude, altitude, tilt, azimuth, albedo, temperature, humidity, cloudCover):
    ...
    G = get_G(when, latitude, longitude, temperature, humidity, cloudCover)
    Gb = get_Gb(when, latitude, longitude, temperature, humidity, cloudCover)
    Gd = get_Gd(when, latitude, longitude, temperature, humidity, cloudCover)
...
def get_GtHDKR(when, latitude, longitude, altitude, tilt, azimuth, albedo, temperature, humidity, cloudCover):
